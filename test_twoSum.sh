...
num_cases=${#nums_list[@]}
## Fill num_cases with the number of test cases
pass_count=0

# Loop through each test case
for ((i=0; i<num_cases; i++)); do
    nums=${nums_list[i]}
    ## Fill nums with the current test case
    target=${targets[i]}
    ## Fill target with the current test case
    answer=${answers[i]}
    ## Fill expected with the current test case
